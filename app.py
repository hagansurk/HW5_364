...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/hhsurkHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    newPri = StringField("What is the new priority of the ToDo list item?",validators=[Required()])
    submit = SubmitField('Update')
...
def update(item):
    ...
    if request.method == 'GET':
        n_pri = form.newPri.data
        #print(n_pri)
        it = TodoItem.query.filter_by(description=item).first()
        it.priority = n_pri
        #print(it.priority)
        db.session.commit()
        flash("Updated priority of the "+item)
        return redirect(url_for('all_lists'))
...
def delete(lst):
    ...
    #print(lst)
    dele = TodoList.query.filter_by(title=lst).first()
    #print(dele)
